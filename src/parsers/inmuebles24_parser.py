...
class Inmuebles24Parser(BaseParser):
    """Parser for Inmuebles24.com"""
    
    def __init__(self):
        super().__init__("https://www.inmuebles24.com", "Inmuebles24")
        
    def get_search_url(self, filters: PropertySearchFilters) -> str:
        """Build Inmuebles24 search URL based on filters."""
        params = {}
        ...
        if filters.property_type:
            ...
            if filters.property_type in type_mapping:
                params['type'] = type_mapping[filters.property_type]
        
        # Location
        if filters.city:
            params['location'] = filters.city.lower().replace(' ', '-')
        if filters.neighborhood:
            params['neighborhood'] = filters.neighborhood.lower().replace(' ', '-')
        ...
        base_url = f"{self.base_url}/inmuebles"
        if params:
            return f"{base_url}?{urlencode(params)}"
        return base_url
    
    def parse_listing_page(self, html: str, url: str) -> List[Dict[str, Any]]:
        """Parse Inmuebles24 listing page and extract property links."""
        properties = []
        ...
        try:
            ...
            if not property_cards:
                property_cards = soup.find_all('div', class_='item-card')
            ...
            for card in property_cards:
                try:
                    ...
                    # Title
                    title_elem = card.find(['h2', 'h3'], class_=['posting-title', 'item-title'])
                    ...
                    app_logger.warning(f"Error parsing Inmuebles24 property card: {e}")
                    continue
            
            app_logger.info(f"Found {len(properties)} properties on Inmuebles24 listing page")
            
        except Exception as e:
            app_logger.error(f"Error parsing Inmuebles24 listing page: {e}")
        
        return properties
    
    def parse_property_detail(self, html: str, url: str) -> Optional[Property]:
        """Parse individual Inmuebles24 property page."""
        try:
            soup = self._get_soup(html)
            ...
            # Title
            title_elem = soup.find('h1', class_=['posting-title', 'item-title'])
            title = title_elem.get_text(strip=True) if title_elem else ""
            ...
            property_obj = Property(
                ...
                source_url=url,
                source_website="inmuebles24.com",
                title=title,
            ...
            app_logger.error(f"Error parsing Inmuebles24 property detail: {e}")
            return None
    
    def get_search_url(self, filters: PropertySearchFilters) -> str:
        """Build search URL based on filters"""
        base_url = "https://www.inmuebles24.com/propiedades"
        params = []
    ...
    def get_total_pages(self, search_url: str) -> int:
        ...
        try:
            ...
            if pagination:
                page_links = pagination.find_all('a', class_='pagination-page')
                if page_links:
            ...
            # Alternative: look for "Última" button
            last_button = soup.find('a', string=re.compile(r'Última|Last'))
            if last_button:
            ...
            if results_summary:
                text = results_summary.get_text()
                # Extract from text like "Mostrando 1-20 de 500 resultados"
    ...
    def _extract_property_id(self, url: str) -> str:
        """Extract property ID from Inmuebles24 URL."""
        # Inmuebles24 URLs format: https://www.inmuebles24.com/inmuebles/123456-title
        match = re.search(r'/inmuebles/(\d+)', url)
        return match.group(1) if match else url.split('/')[-1].split('-')[0]
    
    def _parse_location_from_page(self, soup) -> Optional[Location]:
        """Parse location information from Inmuebles24 page."""
        try:
            location = Location()
            
            # Look for location breadcrumb
            breadcrumb = soup.find('nav', class_='breadcrumb')
            if breadcrumb:
                links = breadcrumb.find_all('a')
                if len(links) >= 2:
                    location.city = links[-2].get_text(strip=True)
                    location.neighborhood = links[-1].get_text(strip=True)
            ...
            for script in scripts:
                if script.string and 'latitude' in script.string and 'longitude' in script.string:
                    try:
            ...
            app_logger.warning(f"Error parsing Inmuebles24 location: {e}")
            return None
    
    def _parse_features(self, soup) -> Optional[PropertyFeatures]:
        """Parse property features from Inmuebles24 page."""
        try:
            ...
            app_logger.warning(f"Error parsing Inmuebles24 features: {e}")
            return None
    
    def _parse_images(self, soup) -> Optional[PropertyImages]:
        """Parse property images from Inmuebles24 page."""
        try:
            ...
            app_logger.warning(f"Error parsing Inmuebles24 images: {e}")
            return None
    
    def _parse_contact(self, soup) -> Optional[PropertyContact]:
        """Parse contact information from Inmuebles24 page."""
        try:
            ...
            app_logger.warning(f"Error parsing Inmuebles24 contact: {e}")
            return None
