...
class NaventParser(BaseParser):
    """Parser for Navent (TokkoApp platform)"""
    
    def __init__(self):
        super().__init__("https://www.navent.com", "Navent")
        
    def get_search_url(self, filters: PropertySearchFilters) -> str:
        """Build Navent search URL based on filters."""
        params = {}
        
        # Operation type mapping
        if filters.operation_type:
            if filters.operation_type == OperationType.SALE:
                params['operation_type'] = '1'  # Sale
            elif filters.operation_type == OperationType.RENT:
                params['operation_type'] = '2'  # Rent
        
        # Property type mapping
        if filters.property_type:
            type_mapping = {
                PropertyType.APARTMENT: '1',  # Departamento
                PropertyType.HOUSE: '2',      # Casa
                PropertyType.COMMERCIAL: '3', # Local comercial
                PropertyType.OFFICE: '4',     # Oficina
                PropertyType.LAND: '5'        # Terreno
            }
        ...
        if filters.currency:
            ...
            if filters.currency in currency_mapping:
                params['currency'] = currency_mapping[filters.currency]
    ...
    def parse_listing_page(self, html: str, url: str) -> List[Dict[str, Any]]:
        """Parse Navent listing page and extract property links."""
        properties = []
        ...
        try:
            ...
            # Find property cards
            property_cards = soup.find_all('div', class_='property-item')
            if not property_cards:
                # Alternative selectors for TokkoApp platform
                property_cards = soup.find_all('article', class_='listing-card')
            if not property_cards:
                property_cards = soup.find_all('div', class_='card-property')
            ...
            for card in property_cards:
                try:
                    ...
                    # Title
                    title_elem = card.find(['h2', 'h3'], class_=['property-title', 'card-title'])
                    ...
                    app_logger.warning(f"Error parsing Navent property card: {e}")
                    continue
            
            app_logger.info(f"Found {len(properties)} properties on Navent listing page")
            
        except Exception as e:
            app_logger.error(f"Error parsing Navent listing page: {e}")
        
        return properties
    
    def parse_property_detail(self, html: str, url: str) -> Optional[Property]:
        """Parse individual Navent property page."""
        try:
            soup = self._get_soup(html)
            ...
            # Title
            title_elem = soup.find('h1', class_=['property-title', 'detail-title'])
            title = title_elem.get_text(strip=True) if title_elem else ""
            ...
            app_logger.error(f"Error parsing Navent property detail: {e}")
            return None
    
    def get_search_url(self, filters: PropertySearchFilters) -> str:
        """Build search URL based on filters"""
        base_url = "https://www.navent.com/propiedades"
        params = []
    ...
    def get_total_pages(self, search_url: str) -> int:
        ...
        try:
            ...
            # Alternative: look for JSON data with pagination info
            script_tags = soup.find_all('script', type='application/json')
            for script in script_tags:
                try:
                    data = json.loads(script.string)
                    if 'pagination' in data:
                        return data['pagination'].get('totalPages', 1)
            ...
            results_count = soup.find('span', class_='results-count')
            if results_count:
                text = results_count.get_text()
                # Extract total results
                match = re.search(r'(\d+)\s+propiedades', text)
                if match:
                    total_results = int(match.group(1))
                    # Assume 24 results per page (common for Navent)
                    return max(1, (total_results + 23) // 24)
    ...
    def _extract_property_id(self, url: str) -> str:
        """Extract property ID from Navent URL."""
        # Navent URLs format: https://www.navent.com/propiedades/123456
        match = re.search(r'/propiedades/(\d+)', url)
        return match.group(1) if match else url.split('/')[-1]
    
    def _parse_location_from_page(self, soup) -> Optional[Location]:
        """Parse location information from Navent page."""
        try:
            location = Location()
            
            # Look for location information
            location_elem = soup.find('div', class_=['location-info', 'property-location'])
            if location_elem:
            ...
            # Look for coordinates in JSON data
            ...
            app_logger.warning(f"Error parsing Navent location: {e}")
            return None
    
    def _parse_features(self, soup) -> Optional[PropertyFeatures]:
        """Parse property features from Navent page."""
        try:
            ...
            app_logger.warning(f"Error parsing Navent features: {e}")
            return None
    
    def _parse_images(self, soup) -> Optional[PropertyImages]:
        """Parse property images from Navent page."""
        try:
            ...
            app_logger.warning(f"Error parsing Navent images: {e}")
            return None
    
    def _parse_contact(self, soup) -> Optional[PropertyContact]:
        """Parse contact information from Navent page."""
        try:
            contact = PropertyContact()
            
            # Look for contact information
            contact_section = soup.find('div', class_=['contact-info', 'agent-info'])
            if contact_section:
                # Agent name
                ...
                # Website
                website_elem = contact_section.find('a', class_='website-link')
                if website_elem:
                    contact.website = website_elem.get('href', '')
            
            return contact
            
        except Exception as e:
            app_logger.warning(f"Error parsing Navent contact: {e}")
            return None
